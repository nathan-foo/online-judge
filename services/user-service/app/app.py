...
# models imported for its create_all side effect (registers User on Base.metadata)
from . import models, user_service  # noqa: F401  # pylint: disable=unused-import

try:
    with open("/run/secrets/clerk_webhook_secret", encoding="utf-8") as f:
...
async def clerk_webhook(request: Request, session: AsyncSessionDep):
    try:
        event = Webhook(CLERK_WEBHOOK_SECRET).verify(
    ...
    if event["type"] in ("user.created", "user.updated"):
        ...
        if not email:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Missing primary email"
            )
        await user_service.sync_user(
            session,
            UserCreate(
...
async def update_me(
    user_in: UserUpdate, current_user: CurrentUserDep, session: AsyncSessionDep
):
